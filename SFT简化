...
def test():
    ...
    sqa_id_list_ST = model.generate(sq_idAtten_ST.input_ids, attention_mask=sq_idAtten_ST.attention_mask, max_new_tokens=1000, do_sample=True, temperature=0.7, top_p=0.9, pad_token_id=tokenizer.pad_token_id)
    sqa_ST = tokenizer.decode(sqa_id_list_ST[0], skip_special_tokens=False)
    print(f'******role={role}\t输出******\n{sqa_ST}\n')
